...
router.get('/order/detail/:id',(request, response,next) =>{
    ...
    db_connection.query('SELECT * FROM orderdetail WHERE orderdetailid=$1',[id],(error, result)=>{
        if(error){
            return next(error);
        }
        response.status(200).json(result.rows);
...
router.post('/order/detail/add',(request, response, next) =>{
    ...
    db_connection.query('INSERT INTO orderdetail (productid, orderid, quantity, peritemprice) VALUES($1,$2,$3,$4)',
    [productid,orderid,quantity,peritemprice], (error, result) =>{
        if(error){
            return next(error);
        }
...
router.put('/order/detail/update', (request, response, next) =>{
    const{productid,orderid,quantity, peritemprice,orderdetailid} = request.body;
    db_connection.query("UPDATE orderdetail SET productid=$1, orderid=$2, quantity=$3, peritemprice=$4 WHERE orderdetailid =$5",
    [productid,orderid,quantity,peritemprice,orderdetailid], (error, result) =>{
        if(error){
            return next(error)
        }
...
router.delete('/order/detail/delete', (request, response, next)=>{
    ...
    db_connection.query("DELETE FROM orderdetail WHERE orderdetailid=$1",[orderdetailid], (error, result)=>{
        if(error){
            return next(error);
        }
