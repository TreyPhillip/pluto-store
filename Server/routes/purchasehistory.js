...
router.post('/purchasehistory/add', (request,response,next)=>{
    const{buyerid, sellerid, shippingaddressid,productid, datepurchased,productprice,quantity} = request.body;
...
router.put('/purchasehistory/update', (request,response,next)=>{
    ...
    db_connection.query('UPDATE purchasehistory SET buyerid=$1, sellerid=$2, shippingaddressid=$3,productid=$4,datepurchased=$5,productprice=$6,quantity=$7'
    ,[buyerid,sellerid,shippingaddressid,productid,datepurchased,productprice,quantity], (error,result) =>{
            if(error){
                return next(error);
            }
